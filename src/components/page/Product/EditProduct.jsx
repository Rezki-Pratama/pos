...
const [product, setProduct] = useState({
        ...
        deskripsi: props.location.state.deskripsi,
        harga: props.location.state.harga,
        message: '',
        display: 'none',
        bgColor: '#9b2c2c'
    })
...
const editProduct = (id) => {
        ...
        console.log(data)
        axios.put(apiUrl + 'barang', data).then(json => {
            try {
                if(json.status === 200 ) {
                    setProduct({
           ...
           <div className="form mt-16 w-7/12">
                ...
                <form>
                    ...
                    <label className="block mt-4">
                        ...
                        <textarea name="deskripsi" onChange={handleInputChange} value={product.deskripsi}  className="mt-1 text-teal-800 font-semibold block w-full rounded-2xl shadow-md p-2 outline-none focus:border-teal-800 border-solid border-2  bg-gray-300 focus:bg-gray-100" placeholder="Deskripsi"/>
                    </label>
                    <label className="block mt-4">
                    ...
                    <div className="flex justify-end">
                        <button type="button" onClick={()=>editProduct(product.id)} className="mt-4 bg-teal-800 hover:bg-teal-900 rounded-xl shadow-md text-white font-bold px-3 py-1 focus:outline-none">
                            Update
                        </button>
                    </div>
                </form>
